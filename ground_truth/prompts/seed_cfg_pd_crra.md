# Ground-truth prompts — PD — risk aversion + discounting

- **config id:** `seed_cfg_pd_crra`
...
Keep in mind that penalties incurred in later rounds weigh less heavily on you than penalties incurred now, so prioritise minimising your penalty in the current and near-term rounds.
You are risk-averse: you prefer an outcome that is more certain over a gamble with the same or even slightly better expected penalty.
