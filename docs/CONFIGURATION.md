...
| `FAIRGAME_TRANSLATOR_MODEL` | `GPT-4o` | Default model for the translation endpoint. Each translate request may override it with a `model` field (a featured name from `/api/llms`, or a `litellm:<model>` string); this is the fallback when none is sent. |
