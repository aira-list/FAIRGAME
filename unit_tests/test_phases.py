...
class _FakeFairGame:
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.fake_communication_config = kwargs.get("fake_communication_config", None)
...
class TestPhaseSelection(unittest.TestCase):
    ...
    def test_communication_runs_before_belief(self) -> None:
        phases = phases_for_game(
            _FakeFairGame(agents_communicate=True, elicit_beliefs=True)
        )
        types = [type(p) for p in phases]
...
class TestPhaseRunDelegation(unittest.TestCase):
    ...
    def test_communication_phase_calls_execute_communication_phase(self) -> None:
        ...
        round_runner._execute_communication_phase.assert_called_once_with()
        round_runner._execute_belief_phase.assert_not_called()

    def test_belief_phase_calls_execute_belief_phase(self) -> None:
        round_runner = mock.Mock()
        BeliefPhase().run(round_runner)
        round_runner._execute_belief_phase.assert_called_once_with()
        round_runner._execute_communication_phase.assert_not_called()

    def test_choose_phase_returns_round_strategies(self) -> None:
        round_runner = mock.Mock()
        round_runner._execute_choose_phase.return_value = ["strategy1", "strategy2"]
        result = ChoosePhase().run(round_runner)
        self.assertEqual(result, ["strategy1", "strategy2"])
        round_runner._execute_choose_phase.assert_called_once_with()
